...
public:
    ...
    n_chunks(N_CHUNKS_PER_THREAD * c_inst_args.n_threads) {
    };

    // functions
    void correct_errors_in_reads(const C_arg& c_inst_args, const std::string& error_correction_info_file_name, CKMCFile& kmc_file);
    void remove_error_correction_info_files(const C_arg& c_inst_args, const std::string& error_correction_info_file_name);
